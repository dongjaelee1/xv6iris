...
Section UShPipesStage.
    ...
    - iIntros "!>" (p b) "%Hb %Hp".
      ...
      + iDestruct "HF" as "(Hc & Hm & Hd)".
        iApply (pipesN_fire g fc adm LW Hcons Hfc v I Hadmit Hplok pnsN (↑pipeN) (S gen_id)
                  γc γm termw TOKN pdepR (pdep_timeless fc adm I L P gF gG) w s b EX Φ
      ...
      + iDestruct "HF" as "(Hc & Hm & #HT)".
        iApply (pipesN_cstep g fc adm LW Hcons Hfc v I Hadmit Hplok pnsN (S gen_id)
                  γc γm termw TOKN pdepR (pdep_timeless fc adm I L P gF gG) w s (S p') b Φ
  ...
  Lemma stage_echo (ws : list (list (bv 8))) (s0 : Z) (gs : nat -> bv 8)
    ...
    line_ok ws -> echo_argv_bytes ws gs -> L = wl_line (drop 1 ws) -> L <> [] ->
    (0 < nc)%nat ->
  ...
  Proof using HL31 Hadmit Hcons Heq Hfc Hfire Hkill Hplok pnsRegG0.
    intros Hok Hbytes HLw HLne Hn Hpeq Ha0 Hfd1 Hfd2 Hav.
    ...
    { iIntros "!>" (M s1 t1 g1 sts cs pidv rb) "%Hi1 %Hb1 %Hl1 %Hr1 #Hnp".
      ...
      - iIntros "!> (Hw & HsL & Hcw & Hmw)". rewrite /pns_echo_lend.
        ...
        cbn [pns_final snd]. rewrite /pns_lexit.
      ...
      - iApply (pse_echo_image_entry (PS := uprogSG_free) g fc adm LW Hcons Hkill rn Heq Hfc
                  v I Hadmit Hplok L HL31 termw TOKN pdepR (pdep_timeless fc adm I L P gF gG)
    ...
    { rewrite /UkShDiag.ush_execfail_law.
      iApply (exf_writer (WLeft 0) dg_execL EchoDisc.alt_execfail 17%nat
                (EXf nc L (WLeft 0) dg_execL) Cr (side_L (P 0)) Cd Hw0 ltac:(lia)
                ...
                (Hfire (WLeft 0) dg_execL Hw0 (or_introl eq_refl))
    ...
    - iIntros "!> [HsL Hf]". iRight. iLeft. iFrame "HsL". rewrite /lrep.
      iExists (Some dg_execL). iFrame "Hf". by iLeft.
  ...
  Lemma stage_mid (k' a b : nat) (s0 : Z) (gs : nat -> bv 8)
      ...
      (ld : list fdstate) (av : nat) :
    UkShCat.cat_argv_bytes a b gs -> L <> [] -> (S k' < nc)%nat ->
  ...
  Proof using HL31 Hadmit Hcons Heq Hfc Hfire Hkill Hplok pnsRegG0 uartGhostG0.
    intros Hbytes HLne Hk Hpeq Ha0 Hfd Hav.
    iIntros "#Hinv #Hslot #Hcode #Hjt #Hcmd Hsz Hstd Hcwd Hch Hraw Hrun".
    pose proof (ukn_const_of_eq N' _ Hpeq (fun x y => eq_refl)) as Hc.
    ...
    { rewrite /pns_kit. iSplit.
      - iExists (EXf nc L (WLeft (S k')) cat_dg_write). iSplitR.
        + iPureIntro. apply (Hfire (WLeft (S k')) cat_dg_write Hwk).
          cbn [fire_src]. right. split; [lia | reflexivity].
        + iApply (pexcl_left fc adm I L P gF gG (S k') cat_dg_write Hk
                    ltac:(vm_compute; discriminate) HLne with "Hpk").
    ...
    - (* ---- THE EXEC-FAILED LAW: the stage's family writer ---- *)
      iApply (exf_writer (WLeft (S k')) dg_execR alt_execR 16%nat
                (EXf nc L (WLeft (S k')) dg_execR) Cr (side_L (P (S k'))) Cd Hwk ltac:(lia)
                ltac:(intros p b' Hp Hb; exact (dg_app_lookup dg_execR u_prompt p b'
                               ltac:(rewrite dg_execR_len; lia) Hb))
                (Hfire (WLeft (S k')) dg_execR Hwk (or_introl eq_refl))
                ltac:(intros c Hcx;
    ...
    - iIntros "!> [HsL Hf]". iRight. iLeft. iFrame "HsL". rewrite /lrep.
      iExists (Some dg_execR). iFrame "Hf". by iLeft.
    - iApply (UserChildren.uch_any_of with "Hch").
    - rewrite /Cr. iFrame.
  Qed.
  ...
  Lemma stage_last (m a b : nat) (s0 : Z) (gs : nat -> bv 8)
      (N' : uk_names Σ) (h' : CpuId) (m' : regfile) (γp : pipe_names) (q szv : Z)
      (ld : list fdstate) (av : nat) :
    UkShCat.cat_argv_bytes a b gs -> L <> [] -> nc = S m ->
    ...
    mWP (Loop : expr riscv_lang).
  Proof using HL31 Hadmit Hcons Heq Hfc Hfire Hkill Hplok pnsRegG0 uartGhostG0.
    intros Hbytes HLne Hm Hpeq Ha0 Hfd Hav.
    iIntros "#Hinv #Hslot #Hcode #Hjt #Hcmd Hsz Hstd Hcwd Hch Hraw Hrun".
    pose proof (ukn_const_of_eq N' _ Hpeq (fun x y => eq_refl)) as Hc.
    ...
    assert (Hfd2 : UkSh.ush_fd2p ld) by (destruct Hfd as (_ & _ & Hf2); exact Hf2).
    iAssert (pkitR WLast L) as "#HkL".
    { rewrite /pns_kit. iSplit.
      - iExists (EXf nc L WLast L). iSplitR.
        + iPureIntro. exact (Hfire WLast L Hwl (or_introl eq_refl)).
        + iApply (pexcl_last fc adm I L P gF gG L HLne HLne with "Hinvs").
      - iPureIntro. intros c [Hc0 Hcl].
        apply (cstep_okN_tok fc adm I Hadmit WLast L c Hc0 Hcl).
        intros j Hj; discriminate Hj. }
    iPoseProof (pdep_last_of_lb fc adm I L P gF gG HLne ltac:(lia) with "Hsn Hinvs")
      as "#Hdw".
    ...
    - (* ---- THE EXEC SUPPLY, AT C6'S ENTRY (fd 2 mute) ---- *)
      ...
      + iIntros "!> (Hr & HsR & Hcw & Hmw)". rewrite /pns_copy_lend_m.
        ...
        { rewrite /pns_sink. iSplitR; [by iPureIntro |]. iFrame "Hinv HkL".
          iSplitR; [iExact "Hdw" |]. cbn [pns_cmode]. iFrame "Hcw Hmw". }
        rewrite /pns_xkQ. iIntros "[#HT | (_ & Hf1 & _)]"; [by iLeft |].
        cbn [pns_final snd].
        iDestruct "Hf1" as (c) "(#Heof & _ & Hcw & Hmw)".
    ...
    - (* ---- THE EXEC-FAILED LAW: the content writer ---- *)
      iApply (exf_writer WLast dg_execR alt_execR 16%nat
                (EXf nc L WLast dg_execR) Cr (side_R (P m)) Cd Hwl ltac:(lia)
                ltac:(intros p b' Hp Hb; exact (dg_app_lookup dg_execR u_prompt p b'
                               ltac:(rewrite dg_execR_len; lia) Hb))
