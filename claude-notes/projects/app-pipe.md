...
- [~] **SH-PIPE-ROUND** (design §4.2): sh's round at the claim — the
  ...
  **PART A LANDED** (`iris/UkShPipeRound.v`: the child walk at the pipe
  shape, and at the LINE); **PART B STOPPED** at the line type and at the
  `LinkRec` instance — see the findings block; **PART C LANDED** modulo
  `Hprog`.
- [x] **PIPE-ADEQUACY**: `iris/UPipeBootAdequacy.v`, `iris/PipeAssumptions.v`,
  `make audit-pipe{,-only}`; the design page's §0 rewritten as landed;
  `pipe_both_law` reported as the one premise.  LANDED by SH-PIPE-ROUND
  (folded in per its brief's item C).  Audit: EXACTLY echo's fourteen.
  `pipe_both_law` is NOT a premise of the theorem — it lives inside
  `Hprog`.
- [ ] **PIPE-LINK-INST** (NEW, opened by SH-PIPE-ROUND): `PipeLinksLine.v`
  + `PipeLinkInst.v`, the ~2,800-line port of upstream's `FileLinksLine.v`
  (2,081) / `FileLinkInst.v` (760) at `pending_at_p` / `pro_pin_p` /
  `proc_stream_p` / `pro_idx_p`.  NOT needed for the adequacy theorem
  (measured: it lands without it) but a PREREQUISITE of the round —
  `UShRound.v` is stated end to end at `FileLinkInst.file_link_inst_at`.
