(* ===================================================================== *)
(* UkShRedirGtk.v -- THE GENERALISED gettoken, lane SH-PARSE               *)
(* (design/app-file.md SS5.1; the linchpin lane SH-REDIR named).            *)
(*                                                                        *)
(* [UkShParseTok.wp_kshp_gettoken] is scoped by                            *)
(* [UkShParse.ushp_no_symbols], and the redirect line falsifies it, so     *)
(* BOTH [gettoken] calls [parseredirs] makes on such a line -- the one     *)
(* that returns '>' and the one that returns 'a' for the file name -- are  *)
(* outside it.  This file is the whole of gettoken at the widened line     *)
(* model [UkShParseSym.ushs_one]: at most one symbol byte in the line, and *)
(* that byte a '>'.                                                       *)
(*                                                                        *)
(* IT IS TWO LEMMAS AND THE REASON IS ONE LINE OF THE LANDED PROOF.        *)
(*                                                                        *)
(* (1) [wp_kshp_gtk_disp_ns] -- the SWITCH at a cursor whose byte is not a *)
(*     symbol.  [UkShParseTok.wp_kshp_gtk_disp] carries                     *)
(*     [ushp_no_symbols len f] but USES it exactly once, to know the byte  *)
(*     AT THE CURSOR is not a symbol; every other step of its 460 lines is *)
(*     already general.  So this is that walk with the premise it actually *)
(*     needs, and the landed lemma is its instance at a symbol-free line.  *)
(*     (The landed statement is not moved -- the bar forbids it -- so the  *)
(*     two coexist and the walk below calls this one.)                     *)
(*                                                                        *)
(* (2) [UkShRedirTok.wp_kshp_gtk_disp_gt] is the THIRD arm, landed by lane *)
(*     SH-REDIR: the byte IS the '>', and it lands on 0x388 with s5 = 62   *)
(*     and s1 advanced by one.  0x388 is exactly where the NUL arm lands,  *)
(*     so gettoken's tail ([wp_kshp_gtk_388], [wp_kshp_gtk_fin]) is the    *)
(*     same code in all three arms and is walked once.                      *)
(*                                                                        *)
(* (3) [wp_kshp_gettoken_sym] -- gettoken end to end, a THREE-WAY case on  *)
(*     the byte at the blank-scanned cursor [kk]: [kk = len] (the NUL      *)
(*     arm), [kk] is the line's one symbol (the '>' arm), or anything else *)
(*     (the default arm).  Its answer is [UkShParseSym.ushs_gettok_res] /  *)
(*     [_end] / [_fin], whose symbol-free instances are the landed         *)
(*     [UkShParseTok.ushp_gettok_res] / [_end] / [_fin].                    *)
(* ===================================================================== *)
...
Section UkShRedirGtk.
  ...
  (* ---- gettoken, the whole function, at one symbol byte ---------------- *)
  ...
  Proof using .
    ...
    (* THREE ARMS, and the byte at the blank-scanned cursor decides which.
       The '>' arm is [UkShRedirTok.wp_kshp_gtk_disp_gt] (lane SH-REDIR) and
       is taken OUT OF LINE first; the other two are the landed dichotomy,
       driven through [wp_kshp_gtk_disp_ns] -- the same walk as
    ...
    { (* ---- THE '>' ARM ---- *)
      ...
      destruct (Hgtok kk Ekkb Esym) as (Hfkk & Hkk1 & Hfkk1).
      assert (Hres : ushs_gettok_res len f kk = 62).
      { unfold ushs_gettok_res.
        rewrite (bool_decide_eq_true_2 _ Ekkb) Esym Hfkk. exact ushs_gt_val. }
      ...
      iApply (wp_kshp_gtk_disp_gt dq s0 len kk f nn h12 mA
                Hkk1 Hfkk Hfkk1 Hs0 Hs64 Hs1A with "Hcode Hstr Hrun").
      ...
      iApply (wp_kshp_gtk_fin m sp0 spl vals ps 62
