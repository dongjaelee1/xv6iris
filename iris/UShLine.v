...
Section UShLine.
  ...
  (* =================================================================== *)
  (*  S4  R3'S TARGET, AND WHY IT IS NOT A LEMMA YET.                     *)
  (*                                                                      *)
  ...
  (*                                                                      *)
  (*    Rsh := fun _ γd γs => UkShLoop.ushl_dat γd ∗ usz γs SZ             *)
  (*                                                                      *)
  (*  at sh's image size ([UShKernel.sh_kexec_sz] = 0x5000), which is what *)
  (*  [UkShFork.ushf_rest_of_body]'s three [sz] bounds are about.          *)
  (*                                                                      *)
  (*  IT IS NOT A LEMMA IN THIS FILE, AND THE REASON IS THE STATEMENT AND  *)
  (*  NOT THE PROOF.  [sh_pay_rest] quantifies over EVERY [uk_names]       *)
  (*  record and over every position ghost, and sh's body cannot be walked *)
  (*  at an arbitrary record: [UkShFork.ushf_rest_of_body] needs SIX       *)
  (*  things that [UkSh.ush_rest]'s body does not carry, and three of them *)
  (*  are about the record itself.                                        *)
  (*                                                                      *)
  (*    THE RECORD'S OWN, which only the ENTRY can produce (they come off  *)
  (*    the key's text image, exactly as [UShKernel.sh_uexec_slot] already *)
  (*    produces [shk_code] and [shk_rodata] through                      *)
  (*    [shk_code_of_text]):  [shk_code γt], [ush_jtab γt] (which carries  *)
  (*    [shk_rodata γt]), and [ukn_const N] -- sh's exit stub answers      *)
  (*    [ukn_pay N xs ∧ ukn_pay N (-1)] out of one resource, and at a      *)
  (*    record whose payload READS the status it cannot.  These are NOT    *)
  (*    another lane's; the fix is that [UkSh.ush_rest] TAKE them, and     *)
  (*    then [wp_ksh_loop] and the four lemmas above it carry [ush_jtab]   *)
  (*    the way they already carry [shk_code], and [sh_uexec_slot] pays    *)
  (*    all three.  That restatement is R3's, and it is what makes         *)
  (*    [sh_pay_rest] payable at all.                                     *)
  (*                                                                      *)
  (*    ANOTHER LANE'S, which stay NAMED:                                 *)
  (*      [UkSh.sh_deps]                 -- [Hsh_owed]'s own conjunct      *)
  (*                                        (write(16) is E5's).           *)
  (*      the EXEC supply                -- [UkRun.uxsup] on the generic   *)
  (*                                        route, and E4's PINNED         *)
  (*                                        [UkShEcho.sh_exec_sup_echo] on *)
  (*                                        the disciplined one (R3(b)):   *)
  (*                                        built from the application     *)
  (*                                        invariant and /echo's pin, so  *)
  (*                                        it cannot be produced here.    *)
  (*      [UkSh.ush_gen_slot N T]        -- E2's: the tainted process's    *)
  (*                                        generic slot, off              *)
  (*                                        [AppEcho.echo_sup_of_taint].   *)
  (*      [UkShMalloc.ushm_sbrk_never_fails] at sh's record                *)
  (*                                     -- the sh lane's standing         *)
  (*                                        assumption ([Hsbrk]).          *)
  (*      [UkShLoop.ush_line_lexable]    -- discharged in tree by          *)
  (*                                        [UkShEcho.ush_line_toks_holds] *)
  (*                                        (E4's), so it is named only    *)
  (*                                        until R3 composes it.          *)
  (*                                                                      *)
  (*  So R3's lemma reads                                                 *)
  (*                                                                      *)
  (*    Lemma sh_rest_holds                                               *)
  ...
  (*      (∀ N γp, ⊢ UkSh.ush_gen_slot (PS := uprogSG_free) N T) ->        *)
  (*      (∀ N, ⊢ UkShEcho.sh_exec_sup_echo (PS := uprogSG_free) N) ->     *)
  (*      ⊢ UInitSh.sh_pay_rest (fun _ γd γs =>                            *)
  (*            UkShLoop.ushl_dat γd ∗ usz γs 0x5000)%I                    *)
  (*                                                                      *)
  (*  and it is NOT in the tree because it is not provable until           *)
  (*  [UkSh.ush_rest] is restated to take the record's own three.  Writing *)
  (*  it as a [Definition ... : Prop] first would be the GAP-PREMISE trap  *)
  (*  (durable-notes, "Vacuity"): a named obligation nothing can ever      *)
  (*  discharge, quoted by a caller as if it could.                        *)
  (* =================================================================== *)
