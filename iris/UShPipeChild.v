...
Section UShPipeChild.
  ...
  (*     forwarded, as [Pex], to [UkShPipeCm.wp_kshp_parsecmd_bar], which  *)
  (*     forwards it again into the whole generic parse cone               *)
  (*     ([UkShPipeEx], [UkShPipePr], [UkShPipeTok], [UkShParse*]) -- none  *)
  (*     of which is this lane's, and every one of which states it as a     *)
  (*     PURE wand.  [UkShPipePaid.wp_kshr_pipe_arm_paid], the other        *)
  (*     callee, does not take it at all.                                  *)
  ...
  Proof using Hpay Hpsok_free ushq_malloc_ok12.
    ...
    (* ---- THE BREAK, off what the parse left (see the note above) ---- *)
    iPoseProof (Hszof with "HM3 Husz") as "Hsz".
    (* ---- THE FAMILY IS BORN HERE: the parse's return is a WP point ---- *)
