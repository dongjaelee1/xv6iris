...
Definition wp_uservec_pt_body `{!riscvGS Σ, !xv6G Σ, !bioslotG Σ, !fileG Σ} `{GEN : GenId} `{CID : CpuId} `{XI : CurCtx}
  ...
  (* ...and THE TEARDOWN'S CLOSE PAYMENTS, owed at every cause because a
     process can be found killed at any trap ([SpecUsertrap.ut_exit_cpay]) *)
  ut_exit_cpay sc_v (tf_of g (ret_pc sepc_v)) sts -∗
