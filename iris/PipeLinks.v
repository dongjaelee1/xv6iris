...
(* ====================================================================== *)
(*  THE BUNDLE.  The pipeline's links are what the record equation gives: *)
(*  every console link of the era is one of the lemmas above once the     *)
(*  console record's claim is [pecl g].  So the bundle a program holds    *)
(*  and spends is the EQUATION ITSELF, as a pure persistent fact, and the *)
(*  links are read off it where they are spent (the leaves below, and     *)
(*  M2's write interface in [PipeLinksLine.pipe_links_gl]).  The record   *)
(*  equations stay where [App.al_echo] hands them over.                   *)
(* ====================================================================== *)
Section pipe_links_bundle.
  Context {Σ : gFunctors}.
  Context `{!echoOutG Σ}.
  Context `{!pipeOutG Σ}.
  Context (g : pipe_gn).
  ...
  Definition pipe_links : iProp Σ :=
    ⌜@riscv_cons_res Σ (@riscv_fixedGS Σ HRg) = pecl g⌝%I.
  ...
  (* the equation, read back out *)
  Lemma pipe_links_eq :
    pipe_links -∗ ⌜@riscv_cons_res Σ (@riscv_fixedGS Σ HRg) = pecl g⌝.
  Proof using . rewrite /pipe_links. by iIntros "%Hc". Qed.

  (* ---- the leaves a consumer spends ---- *)
  Lemma pipe_links_taint : pipe_links -∗ pipe_link_taint.
  Proof using .
    iIntros "Hlk". iDestruct (pipe_links_eq with "Hlk") as %Hc.
    rewrite /pipe_link_taint. iIntros "!>" (k b Φ) "HT HΦ".
    iApply (pipe_write_link_taint g Hc with "HT HΦ").
  Qed.

  Lemma pipe_links_rd : pipe_links -∗ pipe_link_rd.
  Proof using .
    iIntros "Hlk". iDestruct (pipe_links_eq with "Hlk") as %Hc.
    rewrite /pipe_link_rd. iIntros "!>" (k v n ws Φ) "Hpin Hdl HΦ".
    iApply (pipe_read_link g Hc with "Hpin Hdl HΦ").
  Qed.

  Lemma pipe_links_rd_taint : pipe_links -∗ pipe_link_rd_taint.
  Proof using .
    iIntros "Hlk". iDestruct (pipe_links_eq with "Hlk") as %Hc.
    rewrite /pipe_link_rd_taint. iIntros "!>" (k ws Φ) "#HT HΦ".
    iApply (pipe_cons_link_of_taint g Hc with "HT [HΦ]").
    by iApply "HΦ".
  Qed.
  ...
  Proof using .
    ...
    iApply (pipe_file_link g Hc with "Hpin Hpera Ht Hcw Hrlb Hps Hcs HE HPhi");
      try assumption.
  Qed.

  Lemma pipe_links_holds
      (Hcons : @riscv_cons_res Σ (@riscv_fixedGS Σ HRg) = pecl g) :
    ⊢ pipe_links.
  Proof using . by iPureIntro. Qed.
End pipe_links_bundle.

(* PIPE-2W-3: THE BUNDLE IS OPAQUE TO THE INSTANCE SEARCH (kept now that
   the bundle is the record equation: consumers name the instance).  It was
...
(* ONLY THE BUNDLE.  The leaves themselves stay transparent: each is
