...
(* ---- (A2)'s ECHO move, [EchoOut.dl_ok_echo] at [pending_p]: the echo
       leaves the writer owing a whole block, so the clause drops back to
       the completed lines below the one just closed ---- *)
Lemma dl_ok_echo_p (so : postage) (x : list mobs * bv 8)
...
Proof using.
  intros Hao Hweq Hdl. rewrite /dl_ok in Hdl |- *.
  ...
  - assert (HEnil : (snd <$> o_E so) = []).
    { destruct (decide ((snd <$> o_E so) = [])) as [? | Hq]; [done | exfalso].
      apply (pending_at_p_nonnil (o_ps so) (o_cs so) (snd <$> o_E so)
               Hao Hq Hr0).
      rewrite -/(pending_p (o_ps so) (o_cs so) (o_E so)) -Hweq. exact Hw0. }
...
Proof using.
  intros (H1 & _ & H3 & H4 & H5 & H6 & H7 & H8 & H9 & H10 & H11 & H12).
...
Lemma pcl_pure2_arm (k : nat) (ho : list mobs) (so : postage)
  ...
  pcl_pure2 k ho so r pre opn H -> ch_arm_era_p k ho H.
Proof using.
...
Proof using.
  intros [(_ & (_ & _ & _ & _ & _ & HE & _)) | (_ & (_ & _ & _ & _ & _ & HE & _))];
    exact HE.
Qed.
...
Lemma pcl_pure2_dl_ok (k : nat) (ho : list mobs) (so : postage)
    (r : nat) (pre : list (bv 8)) (opn : bool)
    (H : LogEntryDefs.cons_hist) :
  pcl_pure2 k ho so r pre opn H -> dl_ok so (LogEntryDefs.ch_dl H).
Proof using.
  intros [(_ & (_ & _ & _ & _ & _ & _ & Hd)) | (_ & (_ & _ & _ & _ & _ & _ & Hd))];
    exact Hd.
Qed.
...
Lemma pcl_pure_arm (k : nat) (ho : list mobs) (so : postage)
    (H : LogEntryDefs.cons_hist) :
  pcl_pure k ho so H -> ch_arm_era_p k ho H.
...
Proof using.
  ...
  (* (A1) AT THE CLOSE: the arm's echo IS the byte and a store arm sends
     its byte before it closes (K3), so the entry filed is echoed *)
...
(* THE ARM OPENS, and THIS IS WHERE THE DROP IS REFUTED ([EchoOut.
   ecl_pure_open]'s proof at the pipeline discipline): (K1) with the
   flush refuted, (A1) turning the log's length into the era's echoed
   count, and [cons_drop_ok]'s four arms against D3 and (A2). *)
Proof using.
  ...
  pose proof (pout_pure_move k ho h so (LogEntryDefs.ch_acc H)
...
Proof using.
  intros Hok Hev Hd Hb Hdh Hsh [(Hf & Hc) | (Ht & Ho)].
  - left. split; [exact Hf |].
    exact (pcl_pure_open k ho so H h c cs Hok Hev Hd Hb Hdh Hsh Hc).
  - right. split; [exact Ht |].
    exact (pcl_pure_o_open k ho so r pre H h c cs Hok Hev Hd Hb Hdh Hsh Ho).
Qed.
...
Proof using.
  ...
  split_and!; [exact Hout | exact Hc | exact Hp
              | split_and!; [exact Hlog | exact Hdsc | exact Hbts | exact Hdl
                            | exact HEi | exact HEb | lia | exact Hall]
              | | | exact Hdlok'].
  - rewrite /ch_arm_era_p Ha in Hera. rewrite /ch_arm_era_p.
...
Section pipe_out.
  ...
  Proof using .
    ...
    iDestruct "Hc" as (v w so r gb pre opn tm) "(Hpin & Hpera & Hblk & Hcur & Hrb & Htn & Hcs & Hps & HE & Hdl & Hdll & %Hpure)".
    iExists v, w, so, r, gb, pre, opn, tm. iFrame "Hpin Hpera Hblk Hcur Hrb Htn Hcs Hps HE".
  ...
  Proof using .
    intros Hok Hev Hd Hb Hdh Hsh. rewrite /pecl.
    iIntros "[HT | Hc]"; [by iLeft |]. iRight.
    iDestruct "Hc" as (v w so r gb pre opn tm) "(Hpin & Hpera & Hblk & Hcur & Hrb & Htn & Hcs & Hps & HE & Hdl & Hdll & %Hpure)".
    iExists v, w, so, r, gb, pre, opn, tm. iFrame "Hpin Hpera Hblk Hcur Hrb Htn Hcs Hps HE".
  ...
  Proof using .
    ...
    iDestruct "Hp" as (v w so r gb pre opn tm) "(#Hpin & #Hpera & Hblk & Hcur & Hrb & Htn & Hcs & Hps & HE & Hdl & Hdll & %Hall)".
    iSplitL.
    - iRight. iExists v, w, so, r, gb, pre, opn, tm. iFrame "Hpin Hpera Hblk Hcur Hrb Htn Hcs Hps HE Hdl Hdll". by iPureIntro.
  ...
  Proof using .
    ...
    - rewrite /pecl. iRight.
      iExists v, w, (MkO (o_ps so) (o_cs so) (o_E so) (o_w so ++ [b])), r, gb, pre, opn, tm.
  ...
  Proof using .
    ...
    iMod (turn_update v P (pcount_p (o_ps so) (o_cs so) (o_E so) (o_w so))
            (S P) ltac:(lia) with "Ht Hta") as "[Ht Hta]".
    (* the other cs-GROWING step: it is at the BETWEEN-ROUNDS arm ([opn =
       false], refuted above for an open round), so the claim's authority
       is never the frozen one here either. *)
    iDestruct (pcs_auth v (o_cs so) (opn && tm) ltac:(by rewrite Hfls)
                 with "Hcs") as "Hcs".
    iMod (cs_auth_grow v (o_cs so) a with "Hcs") as "[Hcs #Hcslb2]".
    iDestruct (ps_lb_get with "Hps") as "[Hps #Hpslb2]".
    iMod (blk_auth_grow w (pstream so) b with "Hblk") as "[Hblk _]".
    iDestruct (pcs_of_auth v (o_cs so ++ [a]) (opn && tm)
                 ltac:(by rewrite Hfls) with "Hcs") as "Hcs".
    iModIntro. iSplitR "Ht".
    - rewrite /pecl. iRight.
      iExists v, w, (MkO (o_ps so) (o_cs so ++ [a]) (o_E so) [b]), r, gb, pre, opn, tm.
      ...
      + (* (A2) AT A BLOCK'S FIRST BYTE, paid by the writer's own [inp_lb]:
           the delivered list reaches the whole era's input *)
  ...
  Proof using .
    ...
    rewrite Hfls. cbn [cur_frac andb].
    ...
    - rewrite /pecl. iRight.
      iExists v, w, (MkO (o_ps so) (o_cs so) (o_E so) [b]),
              ...
              (palt_isforkS (palt_of a)).
      cbn [o_ps o_cs o_E o_w andb]. rewrite Hpc2.
  ...
  Proof using .
    ...
    rewrite Htrue. cbn [cur_frac andb].
    ...
    - rewrite /pecl. iRight.
      ...
      cbn [o_ps o_cs o_E o_w andb]. rewrite pcount_p_write -HP.
      rewrite (pstream_write (o_ps so) (o_cs so) (o_E so) (o_w so) b).
      rewrite /ConsLog.cons_step. cbn [LogEntryDefs.ch_dl].
  ...
  Proof using .
    ...
    - rewrite /pecl. iRight.
      ...
      iDestruct (pcs_of_auth v (o_cs so ++ [a]) false eq_refl with "Hcs")
        as "Hcs".
  ...
  Proof using .
    ...
    iModIntro. iSplitR "Ht".
    - rewrite /pecl. iRight.
      iExists v, w, (MkO (o_ps so ++ [a]) (o_cs so) (o_E so) (o_w so ++ [b])), r, gb, pre, opn, tm.
      cbn [o_ps o_cs o_E o_w]. rewrite Hpc2.
  ...
  Proof using .
    ...
    iDestruct "Hp" as (v2 w so r gb pre opn tm) "(#Hpin & #Hpera & Hblk & Hcur & Hrb & Hta & Hcs & Hps & HE & Hdl & Hdll & %Hall)".
    ...
    { rewrite /pecl. iRight. iExists v, w, so, r, gb, pre, opn, tm.
      rewrite /ConsLog.cons_step. cbn [LogEntryDefs.ch_dl].
  ...
  Proof using .
    ...
    iDestruct "Hp" as (v w so r gb pre opn tm) "(#Hpin & #Hpera & Hblk & Hcur & Hrb & Hta & Hcs & Hps & HE & Hdl & Hdll & %Hall)".
    pose proof Hall as Hall0.
    ...
    destruct (sessp_prefix_det2 (o_ps so) ps'
                ...
                (done_of (removelast (ins (open_seg h)))) (snd <$> o_E so)
                Hpsb Hok' HokP Hcs'b HpinP Hbyte Hdi1
                ltac:(intros i _ Hfki; exfalso;
                      by rewrite (alts_pad_p_isforkS (snd <$> o_E so)
                                    (o_cs so) i Hnofk) in Hfki)
                Hnm' Hbelow)
      as (_ & HokPres & HeqP & _).
    ...
    iExists v, w, (MkO (o_ps so) (o_cs so) (o_E so ++ [(open_seg h, c)]) []), r, gb, pre, opn, tm.
    ...
    - exact (dl_ok_echo_p so (open_seg h, c) (LogEntryDefs.ch_dl CH)
               Hcsb Hweq Hdlok).
  Qed.
  ...
  Proof using .
    ...
    - iDestruct "Hp" as (v w so r gb pre opn tm) "(#Hpin & #Hpera & Hblk & Hcur & Hrb & Hta & Hcs & Hps & HE & Hdl & Hdll & %Hall)".
      ...
      { iRight. iExists v, w, so, r, gb, pre, opn, tm. iFrame "Hpin Hpera Hblk Hcur Hrb Hta Hcs Hps HE Hdl Hdll".
        by iPureIntro. }
      iRight. iPureIntro.
      ...
      { (* the ordinary reading *)
        ...
        destruct Hc2 as (_ & Hcsl & _ & _ & _ & _ & _).
  ...
  Proof using .
    ...
    { rewrite /pecl. iRight. iExists v, w, ostage0, 0%nat, gb, [], false, false.
      ...
      iDestruct (pcs_of_auth v [] (false && false) eq_refl with "Hcs")
        as "Hcs".
