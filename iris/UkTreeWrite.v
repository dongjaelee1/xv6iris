...
(*                                                                       *)
(* WHAT THE TEST CANNOT YET BE (the lane's STOP rule): the brief asked    *)
(* for "own -> mkdir -> create -> write -> read-learns".  The first two   *)
(* steps are NOT landable, and the three independent reasons are recorded *)
(* in [TreeMove.v] section 4 -- the shortest of them is that              *)
(* [FsAbsDelta.cre_pre] puts the child's row IN the pre-view, so an       *)
(* owner's create move is create's PARENT LEG alone, whose [own_wf]       *)
(* preservation ([own_wf_ent]) design/user-tree.md section 6 records as   *)
(* PRICED AND NOT TAKEN.  So the test runs the steps that ARE landed:     *)
(* own -> write -> (the deed, moved), and section 3 states how it meets   *)
(* [UkTreeRead.wp_uk_tree_read_learns] at the other end.                  *)
(* ===================================================================== *)
...
(*                                                                       *)
(*  and the one seam between the two halves is a PURE lemma this lane     *)
(*  does not owe a proof of but does owe a name: the read corollary asks  *)
(*  for [resolves_from t' d pl = Some (i, AFile bs')] at the MOVED tree,  *)
(*  and [TreeMove.twrote] gives exactly the two facts that make it        *)
(*  transport (the nodes are the same away from [i], and [i] is a FILE in *)
(*  both, so it can only ever be a path's LAST hop).  A                   *)
(*  [resolves_from_twrote] in [TreeView.v] is the one-induction lemma     *)
(*  that closes it; it is TL-1 territory and is NOT landed here.          *)
(*                                                                       *)
