...
from the kernel ELF.  Only the `rodataChunkN` lists are rewritten; the
...
def main():
    ...
    a = ap.parse_args()
    out = subprocess.run([a.objdump, '-s', '-j', '.rodata', a.kernel],
    ...
    for n, ch in enumerate(chunks):
        ...
        pat = re.compile(rf'(def rodataChunk{n} : List \(Nat × Nat\) := \[\n)(.*?)(\])', re.S)
        assert pat.search(src), f'chunk {n} not found'
        src = pat.sub(lambda m: m.group(1) + body + m.group(3), src, count=1)
    open(a.out, 'w').write(src)
    print(f'{len(addrs)} bytes, {len(chunks)} chunks written to {a.out}')
