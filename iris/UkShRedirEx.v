...
(*ALIASES-BEGIN*)
  ...
  Local Notation wp_kshp_parseredirs_gtn :=
    (UkShRedirPr.wp_kshp_parseredirs_gtn N UMalloc UMalloc' ushp_malloc_ok).
  Local Notation wp_kshp_parseredirs_ns := (UkShRedirPr.wp_kshp_parseredirs_ns N).

  (* stage 4's one Hypothesis, at the type the base file names *)
  (* ===================================================================== *)
  (* §11a THE ROUND THAT LEAVES THE LOOP.                                   *)
  (*                                                                       *)
  (* At a cursor whose blank scan reaches the end of the line: [peek] is 0  *)
  (* because the cursor has run out, [gettoken] answers 0 for the same      *)
  (* reason, and the [c.beqz] at 0x63a leaves for 0x662.  It touches        *)
  (* NEITHER the node nor [s1]/[s2]/[s3], which is why it is stated over    *)
  (* none of them -- and why both arms of the loop below can use it.        *)
  (* ===================================================================== *)
  ...
  Proof using .
    ...
    assert (Hnsk : ((cur + ushp_skipws (len - cur) cur f)%nat < len)%nat ->
              ...
              = false)
      by (rewrite Hstop; intro Hlt; exfalso; lia).
    ...
    iApply (wp_kshp_gettoken_sym h11 n10 dq dw dv ps (fp - 120) (fp - 128)
                   s0 len cur' f (mword_of_int (s0 + Z.of_nat cur'))
                   wq weq (14 + nn)
                   Ha0_10 Ha1_10 Ha2_10 Ha3_10 Hcur' eq_refl Hgtok Hs0 Hs64
                   Hps0 Hps8 Hpssz
    ...
    assert (Hend : cur' = len) by (rewrite Hcure; exact Hstop).
    (* ---- the line is exhausted: gettoken answers 0 ---- *)
    assert (Heend : ushs_gettok_end len f cur' = len)
      by (rewrite Hend; exact (ushs_gettok_end_stop len f)).
    assert (Hres0 : ushs_gettok_res len f cur' = 0)
      by (rewrite Hend; exact (ushs_gettok_res_end len f)).
    assert (Hfin : ushs_gettok_fin len f cur' = len)
      by (rewrite Hend; exact (ushs_gettok_fin_stop len f)).
    rewrite Hres0 in Ha0_11. rewrite Hfin.
    ...
    - iPureIntro. intros r Hr. exact (Hk11 r Hr).
  Qed.
  ...
  Proof using ushp_malloc_ok.
    ...
    induction rest as [| tk rest IH ];
      intros done cur h mc wq weq Hred Hs0 Hs64 Hps0 Hps8 Hpssz
        Hfp0 Hfp8 Hfpl Hfph Hp0 Hp8 Hpsz Hcur Hcnt Hpos Htoks Hnsk
    ...
    (* the loop runs at least one round: the EMPTY goal is refuted *)
    1: { exfalso. cbn [length] in Hpos. lia. }
    all: assert (Hgtok : ushs_gt_ok len f)
      by exact (ushs_gt_ok_redir len f gp fe Hred).
    (* ---- the frame's two out-cells are hygienic wherever the frame is -- *)
    ...
    all: rewrite Eret4.
    all: rewrite (ushs_peek_res_nsym len f
                    (cur + ushp_skipws (len - cur) cur f)%nat 4 ushp_T_arg
                    Hnsk ushp_T_arg_sym) in Ha0_5.
    ...
    all: iApply (wp_kshp_gettoken_sym h11 n10 dq dw dv ps (fp - 120) (fp - 128)
                   s0 len cur' f (mword_of_int (s0 + Z.of_nat cur'))
                   wq weq (14 + (8 + nn))
                   Ha0_10 Ha1_10 Ha2_10 Ha3_10 Hcur' eq_refl Hgtok Hs0 Hs64
                   Hps0 Hps8 Hpssz
    ...
    all: assert (Hk11 : forall r : mword 5, ucallee_saved_idx r = true ->
      ...
      (* ===================================================================== *)
      (* THE SPLIT.  Everything above is the round itself; what differs is     *)
      (* which [parseredirs] closes it.  The tail decides: an ordinary word    *)
      (* still to come means the blank scan lands on a word and the call does  *)
      (* nothing; no tail means it lands on the '>' and the call turns.         *)
      (* ===================================================================== *)
      assert (Hnz : ushp_skipws (len - nxt) nxt f = 0%nat)
        by exact (ushp_skipws_idem len e f Hele).
      destruct rest as [| tk2 rest2 ].
      + (* ---- THE LAST ARGUMENT: parseredirs turns once ---- *)
        assert (Hnxtgp : nxt = gp)
          by exact (ushs_toks_nil_inv len gp e f Hrest).
        assert (Hgp : (nxt + ushp_skipws (len - nxt) nxt f)%nat = gp)
          by (rewrite Hnz; lia).
        iApply (wp_kshp_parseredirs_gtn h25 n19 dq dw dv p ps s0 len nxt gp fe
                  f (ushp_exec_pre s0 p (done ++ [tk]))
                  (mword_of_int (s0 + Z.of_nat nxt)) nn
                  Ha0_19 Ha1_19 Ha2_19 Hnxt eq_refl Hred Hgp Hs0 Hs64
                  Hps0 Hps8 Hpssz
                  with "Hcode Hro HM Hpx Hpay [Hty Hav Hev] Hcur Hstr Hws Hsy Hrun").
        ...
        iIntros (t) "Hcur Hstr Hws Hsy Hrnode Hnode HM' Hpay".
        ...
        (* ---- 0x620  c.mv s1,a0 -- ret = the REDIR node ---- *)
        iApply (wp_uk_cmv N h26 n20 (mword_of_int 0x620) s1_idx
                  a0_idx (mword_of_int t) (24 + (8 + nn))
                  ltac:(unfold unot_sp; vm_compute; discriminate)
                  ltac:(vm_compute; discriminate)
                  ltac:(rewrite Ha0_20; symmetry; exact (ushp_mv_val t))
                  with "[] Hrun").
        { iApply (uis_shp_620 with "Hcode"). }
        iIntros (h27) "Hrun".
        set (n21 := <[Regidx s1_idx
                      := regval_into_reg (mword_of_int t : mword 64)]> n20).
        ...
        assert (Hs1_21 : n21 !!! Regidx s1_idx = mword_of_int t)
          by exact (upd_eq n20 (Regidx s1_idx)
                      (regval_into_reg (mword_of_int t : mword 64))).
        assert (Hs2_21 : n21 !!! Regidx s2_idx
                         = mword_of_int (Z.of_nat (length done) + 1)).
        { rewrite (upd_ne n20 (Regidx s1_idx) (Regidx s2_idx) _
          ...
          rewrite (upd_ne n14 (Regidx s3_idx) (Regidx s2_idx) _
                     ltac:(vm_compute; discriminate)).
          exact Hs2_14. }
        (* ---- and the round that finds the line exhausted ---- *)
        iApply (wp_kshp_pex_end dq dw dv s0 ps fp len f (8 + nn) len h27 n21
                  (mword_of_int (s0 + Z.of_nat cur'))
                  (mword_of_int (s0 + Z.of_nat e))
                  Hgtok ltac:(lia)
                  ltac:(rewrite Nat.sub_diag; cbn [ushp_skipws]; lia)
                  ...
                  ltac:(rewrite (Hk21 s8_idx ltac:(vm_compute; reflexivity)
                                   ...
                                   ltac:(vm_compute; discriminate));
                        exact Hs8v)
                  with "Hcode Hro Hcur Hq Heq Hstr Hws Hsy Hrun").
        iIntros "Hcur Hq Heq Hstr Hws Hsy" (hf mf) "%Hpres Hrun".
        iApply ("Hcont" $! t with
                 "Hnode Hrnode Hcur Hq Heq Hstr Hws Hsy HM' Hpay [] [] [] Hrun").
        * iPureIntro. intros r Hr Hr1 Hr2 Hr3.
          rewrite (Hpres r Hr). exact (Hk21 r Hr Hr1 Hr2 Hr3).
        * iPureIntro.
          rewrite (Hpres s2_idx ltac:(vm_compute; reflexivity)) Hs2_21.
          cbn [length]. f_equal. lia.
        * iPureIntro.
          rewrite (Hpres s1_idx ltac:(vm_compute; reflexivity)). exact Hs1_21.
      + (* ---- AN ORDINARY ARGUMENT STILL TO COME: no turn ---- *)
        assert (Htnxt : ushs_toks len f gp nxt (tk2 :: rest2))
          by exact (ushs_toks_skip len gp f e (tk2 :: rest2) Hele Hrest).
        assert (Hnsk2 : ((nxt + ushp_skipws (len - nxt) nxt f)%nat < len)%nat ->
                  ushp_is_sym (f (nxt + ushp_skipws (len - nxt) nxt f)%nat)
                  = false).
        { intros _.
          destruct (ushs_toks_cons_inv len gp nxt f tk2 rest2 Htnxt)
            as (Hn2 & _ & _).
          exact (ushs_toklen_pos_nosym _ _ f Hn2). }
        iApply (wp_kshp_parseredirs_ns h25 n19 dq dw p ps s0 len nxt f
                  (mword_of_int (s0 + Z.of_nat nxt)) (8 + nn)
                  Ha0_19 Ha1_19 Ha2_19 Hnxt eq_refl Hnsk2 Hs0 Hs64
                  Hps0 Hps8 Hpssz
        ...
        iApply (IH (done ++ [tk]) nxt h27 n21
                  (mword_of_int (s0 + Z.of_nat cur'))
                  (mword_of_int (s0 + Z.of_nat e))
                  Hred Hs0 Hs64 Hps0 Hps8 Hpssz Hfp0 Hfp8 Hfpl Hfph
                  Hp0 Hp8 Hpsz Hnxt
                  ...
                  ltac:(rewrite (Hk21 s11_idx ltac:(vm_compute; reflexivity)
                                   ...
                                   ltac:(vm_compute; discriminate));
                        exact Hs11v)
                  with "Hcode Hro HM Hpx Hpay [Hty Hav Hev] Hcur Hq Heq Hstr Hws Hsy Hrun").
        { rewrite /ushp_exec_pre.
          iSplitR; [ iPureIntro; rewrite ushp_len_app1;
                     cbn [length] in Hcnt; lia | ].
          iSplitR; [ iPureIntro; exact Hp0 | ].
          iSplitR; [ iPureIntro; exact Hp8 | ].
          iFrame "Hav Hev". rewrite /ushp_type_at. iExact "Hty". }
        iIntros (t) "Hnode Hrnode Hcur Hq Heq Hstr Hws Hsy HM' Hpay".
