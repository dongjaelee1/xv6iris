...
- [x] **PIPE-LINK-INST** (design §5.8 STOP B): `PipeLinksLine.v` +
  ...
  LANDED (see the Findings block).  Two of the three parenthesised claims
  are REFUTED at the statement: `lk_ab` still needs the ADMISSIBILITY
  guard (only the file's STATE guard is gone), and `lk_exf` cannot be
  echo's literal `1` (`palt_ok (LPipe ws) (PEcho 1)` is false) — only its
  BYTES are echo's.  `lk_pan` IS the literal `3`.
