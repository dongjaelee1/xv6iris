...
`acquire`, `reparent`, `wakeup`, `release` and `sched`.
    ...
    80002078: bne a5,a0,0x8000208e                 -- p != initproc: enter loop
    ...       (panic("init exiting"), NOT taken)
    80002088: addi s1,8; beq s1,s2,0x8000209c; ld a0,0(s1); beqz a0,back;
...
/-- `&wait_lock` from `auipc a0,0x10; addi a0,a0,752` at `0x800020b0`. -/
theorem kx_wl_addr1 :
    (0x800020b0#64 : BitVec 64) + (BitVec.signExtend 64 (16#20 ++ 0#12) + 752#64) = 0x800123a0#64 := by
  decide

/-- `&wait_lock` from `auipc a0,0x10; addi a0,a0,710` at `0x800020da`. -/
theorem kx_wl_addr2 :
    (0x800020da#64 : BitVec 64) + (BitVec.signExtend 64 (16#20 ++ 0#12) + 710#64) = 0x800123a0#64 := by
  decide
...
theorem kx_fileclose (FC : FsEntry filecloseAddr) (Γ : SchedNames) [ClaimIs (hlc := hlc) GF Γ]
    ...
    kctx c kk ∗ pcIs c 0x8000415e#64 ∗ procsInv Γ ∗ trapCsrs c ∗ cpuClaim c (procAddr j) ∗
      intrRes c ∗
  ...
  rw [hp] at h
  rw [show filecloseAddr = 0x8000415e#64 from rfl] at h
  exact h
...
/-- **The ofile scan** from `0x8000208e` (the `ld a0,0(s1)`) at index `fd`,
...
theorem kx_loop (FC : FsEntry filecloseAddr) (Γ : SchedNames) [ClaimIs (hlc := hlc) GF Γ]
    ...
    (hΨ : ∀ (c' : CPU) (kk : KCtx), kxFrame kk j status spval availval →
        (kctx c' kk ∗ pcIs c' 0x8000209c#64 ∗ procsInv Γ ∗ trapCsrs c' ∗
    ...
    ∀ (n fd : Nat), fd + n + 1 = NOFILE → ∀ (c : CPU) (k : KCtx) (L : List (BitVec 64)),
      ...
      (kctx c k ∗ pcIs c 0x8000208e#64 ∗ procsInv Γ ∗ trapCsrs c ∗ cpuClaim c (procAddr j) ∗
        intrRes c ∗ ([∗list] i ↦ f ∈ L, wordPointsTo (pOfile (procAddr j) i) 8 (DFrac.own 1) f) ∗
        Ψ)
      ⊢ wpLoop (GF := GF) c := by
  -- the body from `0x8000208e`: `ld`, the `beqz` test, `fileclose` (crossing)
  -- and the nulling store, handing the `0x80002088` state (list nulled at
  -- `fd`) to `TT`.
  have hbody : ∀ (fd : Nat) (hfdlt : fd < NOFILE) (L : List (BitVec 64)) (hlen : L.length = NOFILE)
      (TT : ∀ (c'' : CPU) (k'' : KCtx), kxFrame k'' j status spval availval →
        ...
        (kctx c'' k'' ∗ pcIs c'' 0x80002088#64 ∗ procsInv Γ ∗ trapCsrs c'' ∗
          cpuClaim c'' (procAddr j) ∗ intrRes c'' ∗
    ...
    icases BigSepL.bigSepL_insert_acc
      ...
      with ⟨Hcell, Hback⟩
    -- ld a0,0(s1)
    k_step (wp_s_ld c _ 0x8000208e#64 true 0#12 10#5 9#5 (by decide) (by decide)
    ...
    -- beqz a0
    k_step (wp_s_branch c _ 0x80002090#64 true 8184#13 10#5 0#5 (by decide) bop.BEQ)
    ...
    · -- open file: fileclose, then null the slot
      ...
      -- jal fileclose
      k_step (wp_s_jal c _ 0x80002092#64 false 8396#21 1#5 (by decide))
        from (text_instr _ _ _ _ rfl rfl) Htext $$ [- $Hk $Hpc]
        with [KCtx.setReg_sie, KCtx.setReg_proc]
      iintro Hk Hpc
      -- fileclose(a0)
      have hfc := kx_fileclose (hlc := hlc) (GF := GF) FC Γ c
        ((k.setReg 10#5 (L[fd]'hfdlt2)).setReg 1#5 0x80002096#64) j hj
      ...
      have hkframe1 : kxFrame ((k.setReg 10#5 (L[fd]'hfdlt2)).setReg 1#5 0x80002096#64) j status spval availval :=
      ...
      have hjump : jumpPc (((k.setReg 10#5 (L[fd]'hfdlt2)).setReg 1#5 0x80002096#64).regs 1#5)
          = 0x80002096#64 := by
      ...
      ihave Hpc := (show pcIs (GF := GF) c2 (jumpPc (((k.setReg 10#5 (L[fd]'hfdlt2)).setReg 1#5
          0x80002096#64).regs 1#5)) ⊢ pcIs c2 0x80002096#64 from by rw [hjump]) $$ Hpc
      -- sd zero,0(s1)
      k_step (wp_s_sd c2 _ 0x80002096#64 false 0#12 9#5 0#5 (by decide) (L[fd]'hfdlt2))
        from (text_instr _ _ _ _ rfl rfl) Htext $$ [- $Hk $Hpc]
        with [KCtx.rget_withRegs', hR9, kx_add0, KCtx.rget_zero, KCtx.setReg_sie, KCtx.setReg_proc]
      iintro Hk Hpc Hcell
      ihave Hbig := Hback $$ %(0#64) Hcell
      -- j 0x80002088
      k_step (wp_s_j c2 _ 0x8000209a#64 true 2097134#21)
        from (text_instr _ _ _ _ rfl rfl) Htext $$ [- $Hk $Hpc]
        with [KCtx.setReg_sie, KCtx.setReg_proc]
      iintro Hk Hpc
  ...
  | zero =>
    ...
    icases kctx_kernelText _ _ $$ Hk with ⟨#Htext, Hk⟩
    -- addi s1,8
    k_step (wp_s_addi c'' _ 0x80002088#64 true 8#12 9#5 9#5 (by decide))
      from (text_instr _ _ _ _ rfl rfl) Htext $$ [- $Hk $Hpc]
      with [KCtx.setReg_sie, KCtx.setReg_proc]
    iintro Hk Hpc
    -- beq s1,s2 (taken)
    k_step (wp_s_branch c'' _ 0x8000208a#64 false 18#13 9#5 18#5 (by decide) bop.BEQ)
      from (text_instr _ _ _ _ rfl rfl) Htext $$ [- $Hk $Hpc]
...
/-- `reparent`'s contract at its call site `0x80001ffa`, with the `p`
...
theorem kx_reparent (RP : REPARENT) (Γ : SchedNames) (c : CPU) (k' : KCtx)
    ...
    kctx c k' ∗ pcIs c 0x80001ffa#64 ∗ procsInv Γ ∗ initprocIs ip ∗ waitResAt curCtx parents ∗
...
/-- `wakeup`'s contract at its call site `0x80001f94`. -/
...
set_option maxHeartbeats 8000000 in
/-- **`kexit`'s tail** from `0x8000209c`: `begin_op(); iput(p->cwd); end_op();
...
theorem kx_rest (AC : ACQUIRE) (RE : RELEASE) (RP : REPARENT) (WU : WAKEUP) (SC : SCHED) [FsEnv]
    ...
    kctx c k ∗ pcIs c 0x8000209c#64 ∗ procsInv Γ ∗ trapCsrs c ∗ cpuClaim c (procAddr j) ∗ intrRes c ∗
  ...
  icases kctx_kernelText _ _ $$ Hk with ⟨#Htext, Hk⟩
  -- jal begin_op
  k_step (wp_s_jal c _ 0x8000209c#64 false 7176#21 1#5 (by decide))
    from (text_instr _ _ _ _ rfl rfl) Htext $$ [- $Hk $Hpc] with [KCtx.setReg_sie, KCtx.setReg_proc]
  iintro Hk Hpc
  have hf1 : kxFrame (k.setReg 1#5 0x800020a0#64) j status spval availval :=
    kx_setReg_frame k j status spval 1#5 _ hf (by decide) (by decide) (by decide) (by decide)
  iapply (kx_fs beginOpAddr 0x80003ca4#64 rfl FsEnv.begin_op Γ status spval availval c
    (k.setReg 1#5 0x800020a0#64) j hj hf1) $$ [- $Hk $Hpc $Hpinv $Htc $Hclaim $Hres]
  iapply wpNext_intro_pin
  iintro %c1 %hpin1
  iintro %spie1 %spp1 %R1 Hk Hpc Htc Hclaim Hres %hcs1
  have hjp1 : jumpPc ((k.setReg 1#5 0x800020a0#64).regs 1#5) = 0x800020a0#64 := by
    rw [KCtx.setReg_regs, RegMap.set_apply, if_pos rfl]; decide
  ihave Hpc := (show pcIs (GF := GF) c1 (jumpPc ((k.setReg 1#5 0x800020a0#64).regs 1#5)) ⊢
      pcIs c1 0x800020a0#64 from by rw [hjp1]) $$ Hpc
  have hf1 := kxFrame_cross hf1 spie1 spp1 R1 hcs1
  -- ld a0,336(s3)
  k_step (wp_s_ld c1 _ 0x800020a0#64 false 336#12 10#5 19#5 (by decide) (by decide)
      (DFrac.own 1) V.cwd)
    from (text_instr _ _ _ _ rfl rfl) Htext $$ [- $Hk $Hpc]
    with [KCtx.rget_eq, hf1.2.2.2.2.2.2.2.1, kx_pCwd0, KCtx.setReg_sie, KCtx.setReg_proc]
  iintro Hk Hpc Hcwd
  have hf1a : kxFrame (((k.setReg 1#5 0x800020a0#64).withSpie spie1 spp1).withRegs
  ...
  -- jal iput
  k_step (wp_s_jal c1 _ 0x800020a4#64 false 4888#21 1#5 (by decide))
    from (text_instr _ _ _ _ rfl rfl) Htext $$ [- $Hk $Hpc] with [KCtx.setReg_sie, KCtx.setReg_proc]
  iintro Hk Hpc
  have hf1b : kxFrame (((k.setReg 1#5 0x800020a0#64).withSpie spie1 spp1).withRegs
      ((R1.set 10#5 V.cwd).set 1#5 0x800020a8#64)) j status spval availval :=
    kx_setReg_frame _ j status spval 1#5 0x800020a8#64 hf1a
      (by decide) (by decide) (by decide) (by decide)
  iapply (kx_fs iputAddr 0x800033bc#64 rfl FsEnv.iput Γ status spval availval c1 _ j hj hf1b)
    $$ [- $Hk $Hpc $Hpinv $Htc $Hclaim $Hres]
  iapply wpNext_intro_pin
  iintro %c2 %hpin2
  iintro %spie2 %spp2 %R2 Hk Hpc Htc Hclaim Hres %hcs2
  ihave Hpc := (kx_pcIs_jump c2 _ (R1.set 10#5 V.cwd) 0x800020a8#64 (by decide)) $$ Hpc
  have hf2 := kxFrame_cross hf1b spie2 spp2 R2 hcs2
  -- jal end_op
  k_step (wp_s_jal c2 _ 0x800020a8#64 false 7304#21 1#5 (by decide))
    from (text_instr _ _ _ _ rfl rfl) Htext $$ [- $Hk $Hpc] with [KCtx.setReg_sie, KCtx.setReg_proc]
  iintro Hk Hpc
  have hf2a := kx_setReg_frame _ j status spval 1#5 0x800020ac#64 hf2
    (by decide) (by decide) (by decide) (by decide)
  iapply (kx_fs endOpAddr 0x80003d30#64 rfl FsEnv.end_op Γ status spval availval c2 _ j hj ?hfe)
    $$ [- $Hk $Hpc $Hpinv $Htc $Hclaim $Hres]
  case hfe => exact hf2a
  iapply wpNext_intro_pin
  iintro %c3 %hpin3
  iintro %spie3 %spp3 %R3 Hk Hpc Htc Hclaim Hres %hcs3
  ihave Hpc := (kx_pcIs_jump c3 _ R2 0x800020ac#64 (by decide)) $$ Hpc
  have hf3 := kxFrame_cross hf2a spie3 spp3 R3 hcs3
  have h19R3 : R3 19#5 = procAddr j := hf3.2.2.2.2.2.2.2.1
  -- sd zero,336(s3): p->cwd = 0
  k_step (wp_s_sd c3 _ 0x800020ac#64 false 336#12 19#5 0#5 (by decide) V.cwd)
    from (text_instr _ _ _ _ rfl rfl) Htext $$ [- $Hk $Hpc]
    with [KCtx.rget_eq, KCtx.withRegs_regs, h19R3, kx_pCwd0, KCtx.rget_zero, KCtx.setReg_sie, KCtx.setReg_proc]
  iintro Hk Hpc Hcwd
  -- ==== the lock section (0x800020b0 → 0x800020e6) ====
  -- acquire(&wait_lock): auipc a0,0x10; addi a0,a0,752; jal acquire
  k_step (wp_s_auipc c3 _ 0x800020b0#64 false 16#20 10#5 (by decide))
    from (text_instr _ _ _ _ rfl rfl) Htext $$ [- $Hk $Hpc] with [KCtx.setReg_sie, KCtx.setReg_proc]
  iintro Hk Hpc
  k_step (wp_s_addi c3 _ 0x800020b4#64 false 752#12 10#5 10#5 (by decide))
    from (text_instr _ _ _ _ rfl rfl) Htext $$ [- $Hk $Hpc]
    with [KCtx.rget_eq, kx_wl_addr1, KCtx.setReg_sie, KCtx.setReg_proc]
  iintro Hk Hpc
  k_step (wp_s_jal c3 _ 0x800020b8#64 false 2091778#21 1#5 (by decide))
    from (text_instr _ _ _ _ rfl rfl) Htext $$ [- $Hk $Hpc] with [KCtx.setReg_sie, KCtx.setReg_proc]
  iintro Hk Hpc
  -- acquire(&wait_lock) at 0x80000bba
  have hacw : ∀ (k' : KCtx) (hsie' : k'.sie = false) (hnoff' : k'.noff + 1 < 2 ^ 31)
      (hK' : 10 ≤ k'.avail) (hs' : "wait_lock" ∉ k'.locks) (ha0 : k'.regs 10#5 = waitLockAddr),
      kctx c3 k' ∗ pcIs c3 0x80000bba#64 ∗ isLock γw waitLockAddr "wait_lock" waitLockPay ∗
      (∀ R' : RegMap,
  ...
  ihave Hpc := (kx_pcIs_jump c3 _ _ 0x800020bc#64 (by decide)) $$ Hpc
  ...
  -- c.mv a0,s3
  k_step (wp_s_add c3 _ 0x800020bc#64 true 10#5 0#5 19#5 (by decide))
    from (text_instr _ _ _ _ rfl rfl) Htext $$ [- $Hk $Hpc]
    with [KCtx.rget_eq, hW19, KCtx.rget_zero]
  iintro Hk Hpc
  -- jal reparent
  k_step (wp_s_jal c3 _ 0x800020be#64 false 2096956#21 1#5 (by decide))
    from (text_instr _ _ _ _ rfl rfl) Htext $$ [- $Hk $Hpc] with [KCtx.setReg_sie, KCtx.setReg_proc]
  iintro Hk Hpc
  -- reparent(p)
  ...
  subst c5
  ihave Hpc := (kx_pcIs_jump c3 _ _ 0x800020c2#64 (by decide)) $$ Hpc
  ...
  k_step (wp_s_ld c3 _ 0x800020c2#64 false 56#12 10#5 19#5 (by decide) (by decide) (DFrac.own 1)
  ...
  -- jal wakeup
  k_step (wp_s_jal c3 _ 0x800020c6#64 false 2096846#21 1#5 (by decide))
    from (text_instr _ _ _ _ rfl rfl) Htext $$ [- $Hk $Hpc] with [KCtx.setReg_sie, KCtx.setReg_proc]
  iintro Hk Hpc
  -- wakeup(p->parent)
  ...
  subst c6
  ihave Hpc := (kx_pcIs_jump c3 _ _ 0x800020ca#64 (by decide)) $$ Hpc
  ...
  k_step (wp_s_add c3 _ 0x800020ca#64 true 10#5 0#5 19#5 (by decide))
    from (text_instr _ _ _ _ rfl rfl) Htext $$ [- $Hk $Hpc]
    with [KCtx.rget_eq, hd6_19, KCtx.rget_zero]
  iintro Hk Hpc
  k_step (wp_s_jal c3 _ 0x800020cc#64 false 2091758#21 1#5 (by decide))
    from (text_instr _ _ _ _ rfl rfl) Htext $$ [- $Hk $Hpc] with [KCtx.setReg_sie, KCtx.setReg_proc]
  iintro Hk Hpc
  ...
  have hacp : ∀ (k' : KCtx) (hsie' : k'.sie = false) (hnoff' : k'.noff + 1 < 2 ^ 31)
      ...
      kctx c3 k' ∗ pcIs c3 0x80000bba#64 ∗ isLock (Γ.lock j) (procAddr j) "proc" (procLockPay Γ j) ∗
      (∀ R' : RegMap,
  ...
  ihave Hpc := (kx_pcIs_jump c3 _ _ 0x800020d0#64 (by decide)) $$ Hpc
  ...
  k_step (wp_s_sw c3 _ 0x800020d0#64 false 44#12 19#5 20#5 (by decide) xs)
    from (text_instr _ _ _ _ rfl rfl) Htext $$ [- $Hk $Hpc]
    with [KCtx.rget_eq, hR7_19, kx_pXstate]
  iintro Hk Hpc Hxstate
  -- c.li a5,5
  k_step (wp_s_addi c3 _ 0x800020d4#64 true 5#12 15#5 0#5 (by decide))
    from (text_instr _ _ _ _ rfl rfl) Htext $$ [- $Hk $Hpc] with [KCtx.rget_eq]
  iintro Hk Hpc
  -- sw a5,24(s3): p->state = ZOMBIE
  k_step (wp_s_sw c3 _ 0x800020d6#64 false 24#12 19#5 15#5 (by decide) RUNNING)
    from (text_instr _ _ _ _ rfl rfl) Htext $$ [- $Hk $Hpc]
  ...
  k_step (wp_s_auipc c3 _ 0x800020da#64 false 16#20 10#5 (by decide))
    from (text_instr _ _ _ _ rfl rfl) Htext $$ [- $Hk $Hpc] with [KCtx.setReg_sie, KCtx.setReg_proc]
  iintro Hk Hpc
  k_step (wp_s_addi c3 _ 0x800020de#64 false 710#12 10#5 10#5 (by decide))
    from (text_instr _ _ _ _ rfl rfl) Htext $$ [- $Hk $Hpc]
    with [KCtx.rget_eq, kx_wl_addr2, KCtx.setReg_sie, KCtx.setReg_proc]
  iintro Hk Hpc
  k_step (wp_s_jal c3 _ 0x800020e2#64 false 2091872#21 1#5 (by decide))
    from (text_instr _ _ _ _ rfl rfl) Htext $$ [- $Hk $Hpc] with [KCtx.setReg_sie, KCtx.setReg_proc]
  iintro Hk Hpc
  have hrew : ∀ (k' : KCtx) (hsie' : k'.sie = false) (hnoff' : 1 ≤ k'.noff)
      (hK' : 10 ≤ k'.avail) (hreen0 : k'.noff ≠ 1) (ha0 : k'.regs 10#5 = waitLockAddr),
      kctx c3 k' ∗ pcIs c3 0x80000c42#64 ∗ isLock γw waitLockAddr "wait_lock" waitLockPay ∗
  ...
  ihave Hpc := (kx_pcIs_jump c3 _ _ 0x800020e6#64 (by decide)) $$ Hpc
  -- jal sched
  k_step (wp_s_jal c3 _ 0x800020e6#64 false 2096474#21 1#5 (by decide))
    from (text_instr _ _ _ _ rfl rfl) Htext $$ [- $Hk $Hpc] with [KCtx.setReg_sie, KCtx.setReg_proc]
  iintro Hk Hpc
  ...
  have hsc : ∀ (k' : KCtx) (ch' : BitVec 64) (hK' : schedSlots ≤ k'.avail) (hsie' : k'.sie = false)
      ...
      kctx c3 k' ∗ pcIs c3 0x80001e40#64 ∗ procsInv Γ ∗ procHeld Γ c3 j ZOMBIE ch' ∗
...
/-- `&initproc` from `auipc a5,0x8; ld a5,536(a5)` at `0x80002068`. -/
theorem kx_initproc_addr :
    (0x80002068#64 : BitVec 64) + (BitVec.signExtend 64 (8#20 ++ 0#12) + 536#64) = initprocAddr := by
  decide
...
theorem kexit_proof (MP : MYPROC) (AC : ACQUIRE) (RE : RELEASE) (RP : REPARENT) (WU : WAKEUP)
  ...
  k_step (wp_s_push cpu _ 0x80002050#64 true 4048#12 6 hK6 kx_imm_m48)
    from (text_instr _ _ _ _ rfl rfl) Htext $$ [- $Hk $Hpc]
  iintro Hk Hpc Hframe
  irevert Hframe
  stack_cells
  iintro ⟨⟨%w0, F0⟩, ⟨%w1, F1⟩, ⟨%w2, F2⟩, ⟨%w3, F3⟩, ⟨%w4, F4⟩, ⟨%w5, F5⟩, _⟩
  k_step (wp_s_sd cpu _ 0x80002052#64 true 40#12 2#5 1#5 (by decide) w0)
    from (text_instr _ _ _ _ rfl rfl) Htext $$ [- $Hk $Hpc]
  iintro Hk Hpc F0
  k_step (wp_s_sd cpu _ 0x80002054#64 true 32#12 2#5 8#5 (by decide) w1)
    from (text_instr _ _ _ _ rfl rfl) Htext $$ [- $Hk $Hpc]
  iintro Hk Hpc F1
  k_step (wp_s_sd cpu _ 0x80002056#64 true 24#12 2#5 9#5 (by decide) w2)
    from (text_instr _ _ _ _ rfl rfl) Htext $$ [- $Hk $Hpc]
  iintro Hk Hpc F2
  k_step (wp_s_sd cpu _ 0x80002058#64 true 16#12 2#5 18#5 (by decide) w3)
    from (text_instr _ _ _ _ rfl rfl) Htext $$ [- $Hk $Hpc]
  iintro Hk Hpc F3
  k_step (wp_s_sd cpu _ 0x8000205a#64 true 8#12 2#5 19#5 (by decide) w4)
    from (text_instr _ _ _ _ rfl rfl) Htext $$ [- $Hk $Hpc]
  iintro Hk Hpc F4
  k_step (wp_s_sd cpu _ 0x8000205c#64 true 0#12 2#5 20#5 (by decide) w5)
    from (text_instr _ _ _ _ rfl rfl) Htext $$ [- $Hk $Hpc]
  iintro Hk Hpc F5
  k_step (wp_s_addi cpu _ 0x8000205e#64 true 48#12 8#5 2#5 (by decide))
    from (text_instr _ _ _ _ rfl rfl) Htext $$ [- $Hk $Hpc]
  iintro Hk Hpc
  -- c.mv s4,a0: s4 = status (= a0 at entry)
  k_step (wp_s_add cpu _ 0x80002060#64 true 20#5 0#5 10#5 (by decide))
    from (text_instr _ _ _ _ rfl rfl) Htext $$ [- $Hk $Hpc] with [KCtx.rget_eq, KCtx.rget_zero]
  iintro Hk Hpc
  -- jal myproc
  k_step (wp_s_jal cpu _ 0x80002062#64 false 2095224#21 1#5 (by decide))
    from (text_instr _ _ _ _ rfl rfl) Htext $$ [- $Hk $Hpc]
  iintro Hk Hpc
  -- myproc()
  ...
  have hret66 : jumpPc 0x80002066#64 = 0x80002066#64 := by decide
  k_norm [hret66]
  ...
  k_step (wp_s_add cpu _ 0x80002066#64 true 19#5 0#5 10#5 (by decide))
  ...
  k_step (wp_s_auipc cpu _ 0x80002068#64 false 8#20 15#5 (by decide))
    from (text_instr _ _ _ _ rfl rfl) Htext $$ [- $Hk $Hpc]
  iintro Hk Hpc
  iapply (wp_s_ld cpu _ 0x8000206c#64 false 536#12 15#5 15#5 (by decide) (by decide)
  ...
  k_step (wp_s_addi cpu _ 0x80002070#64 false 208#12 9#5 10#5 (by decide))
    from (text_instr _ _ _ _ rfl rfl) Htext $$ [- $Hk $Hpc] with [KCtx.rget_eq, h10]
  iintro Hk Hpc
  -- addi s2,a0,336: s2 = &cwd
  k_step (wp_s_addi cpu _ 0x80002074#64 false 336#12 18#5 10#5 (by decide))
    from (text_instr _ _ _ _ rfl rfl) Htext $$ [- $Hk $Hpc] with [KCtx.rget_eq, h10]
  iintro Hk Hpc
  -- bne a5,a0: p != initproc, so jump to the loop at 0x8000208e
  k_step (wp_s_branch cpu _ 0x80002078#64 false 22#13 15#5 10#5 (by decide) bop.BNE)
    from (text_instr _ _ _ _ rfl rfl) Htext $$ [- $Hk $Hpc]
