...
(the uncounted mode).  Two contracts: over a raw table (`do_free = 0`,
the fixed mappings of `proc_pagetable`/`proc_freepagetable`) and over an
address space (`do_free = 1`).  Needs 22 of the caller's stack slots
(8 + `kfree`'s 14).
