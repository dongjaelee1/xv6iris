...
def main():
    ...
    print("definitions a reader must read. The proof cone is excluded.")
    print()
    ...
    print()
    print("<details><summary>Caveats</summary>")
    print()
    print("* **Inductives are not counted.** `printer.mli`'s `context_object` has no")
    print("  inductive case, so datatype declarations are traversed but never printed.")
    print("  `FsDurSnap.snap_bytes`, the 21-field `Record` behind the durability")
    print("  guarantee, is absent here; `--with-inductives` bounds that component.")
    print("* **Opaque/Transparent is a proxy** for *inhabits a `Prop`*, not a synonym.")
    print("  All 43 in-tree opaque constants were `Prop`-valued when last checked.")
    print("* **Lines are vernac sentences**, not the comments around them.")
    print("* **Report, not check** — no baseline is diffed. A jump in the file count")
    print("  means a statement-level definition reached a new part of the tree.")
    print("* **Do not read containment off the file column.** `xv6_power_adequacy_xv6Σ`")
    print("  and `xv6_fs_adequacy_xv6Σ` share most of their files and neither")
    print("  definition set contains the other.")
    print()
    print("</details>")
    if unresolved:
        print()
